...
#Another conclusion is that when all consumers stop then when they reconnects they will read the messages from the position of consumer offset, so if messages are stacked then all those will be read and consumer offset will be updated to a new position in order to not to read those messages again.
